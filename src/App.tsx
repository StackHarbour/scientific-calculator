...
function App() {
  ...
  const [dark, setDark] = useState(() => {
    const saved = localStorage.getItem('scientific-calculator-theme')
  ...
  useEffect(() => {
    ...
    localStorage.setItem('scientific-calculator-theme', dark ? 'dark' : 'light')
  }, [dark])
          ...
          <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            <div className="border-b border-zinc-200 p-5 dark:border-zinc-800 sm:p-7">
              <input
                ...
                placeholder="Type or paste an expression, e.g. (1/2) + 3/4"
                aria-label="Mathematical expression"
                spellCheck={false}
                autoComplete="off"
                className="display-scroll block w-full min-h-8 overflow-x-auto bg-transparent text-right text-sm text-zinc-500 outline-none placeholder:text-zinc-400 dark:placeholder:text-zinc-600"
              />
              ...
              <div className="mt-3 flex justify-between gap-2">
                <span className="self-center text-xs text-zinc-400">Paste with Ctrl+V • Fractions/division: x/y</span>
              ...
              <div className="grid grid-cols-5 gap-2">
                {keyRows.flat().map((key, i) => key === '' ? <div key={i} /> : (
                  <button
                    key={`${key}-${i}`}
                    onClick={() => press(key)}
                    className={`rounded-xl border text-sm font-medium transition active:scale-[.98] ${key === '='
                      ? 'min-h-16 text-lg border-zinc-950 bg-zinc-950 text-white hover:bg-zinc-800 dark:border-white dark:bg-white dark:text-zinc-950 dark:hover:bg-zinc-200'
                      : 'min-h-12'} ${['÷', '×', '−', '+'].includes(key) ? 'border-zinc-200 bg-zinc-100 text-zinc-900 hover:bg-zinc-200 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white' :
                        ['AC', '⌫'].includes(key) ? 'border-zinc-200 bg-white text-red-500 hover:bg-red-50 dark:border-zinc-800 dark:bg-zinc-950 dark:hover:bg-red-950/30' :
                          key !== '=' ? 'border-zinc-200 bg-white hover:bg-zinc-100 dark:border-zinc-800 dark:bg-zinc-950 dark:hover:bg-zinc-800' : ''
                      }`}
                  >
                    {key}
                  </button>
