...
function findIdentifiers(expression: string) {
  const withoutScientificNotation = expression.replace(/(?:\d+(?:\.\d*)?|\.\d+)[eE][+-]?\d+/g, '')
...
export function evaluateExpression(input: string, mode: AngleMode): string {
  ...
  const allowed = new Set([
    ...
    'cot_angle', 'sec_angle', 'csc_angle', 'asin_angle', 'acos_angle', 'atan_angle',
    'sinh', 'cosh', 'tanh', 'asinh', 'acosh', 'atanh', 'sqrt', 'abs', 'floor',
    'ceil', 'exp', 'factorial', 'combinations', 'permutations', 'nthRoot',
  ])
  for (const identifier of findIdentifiers(expr)) {
...
export function calculateFraction(a: number, b: number, operation: '+' | '-' | '*' | '/') {
  if (!Number.isFinite(a) || !Number.isFinite(b)) throw new Error('Enter valid numerator and denominator values.')
  if (b === 0) throw new Error('A denominator cannot be zero.')
